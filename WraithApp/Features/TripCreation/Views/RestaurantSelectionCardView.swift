//
//  RestaurantSelectionCardView.swift
//  WraithApp
//
...
final class RestaurantSelectionCardView: BaseCardView, TripCreationCardUpdating {
    ...
    init(viewModel: TripCreationViewModel) {
        ...
        super.init(title: "Restoranlar", accessoryView: button)
    ...
    func update(with draft: TripCreationDraft) {
        ...
        seeAllButton.isEnabled = !restaurants.isEmpty
        ...
        for (index, cardView) in itemCardViews.enumerated() {
            ...
            cardView.configure(with: POIDisplayItem(
                ...
                isSelected: draft.selectedRestaurantIDs.contains(restaurant.id)
            ))
        }
    }
    ...
    private func didTapItemCard(at index: Int) {
        guard index < viewModel.restaurants.count else { return }
        viewModel.toggleRestaurantSelection(viewModel.restaurants[index])
    }

    @objc private func didTapSeeAll() {
        guard let city = viewModel.selectedCity?.name, !viewModel.restaurants.isEmpty else { return }
        guard let presenter = ParentViewController else { return }

        let allRestaurantsViewController = AllRestaurantsViewController(city: city, viewModel: viewModel)

        if let navigationController = presenter.navigationController {
            navigationController.pushViewController(allRestaurantsViewController, animated: true)
        } else {
            presenter.present(UINavigationController(rootViewController: allRestaurantsViewController), animated: true)
        }
    }
}
