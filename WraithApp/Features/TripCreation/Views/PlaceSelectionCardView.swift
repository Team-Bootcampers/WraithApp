...
final class PlaceSelectionCardView: BaseCardView, TripCreationCardUpdating {
    ...
    init(viewModel: TripCreationViewModel) {
        ...
        super.init(title: "Gezilecek Yerler", accessoryView: button)
    ...
    func update(with draft: TripCreationDraft) {
        ...
        if places.isEmpty {
            placeholderLabel.text = "Gezilecek yerler yükleniyor..."
        ...
        seeAllButton.isEnabled = !places.isEmpty
        ...
        for (index, cardView) in itemCardViews.enumerated() {
            ...
            cardView.configure(with: POIDisplayItem(
                ...
                isSelected: draft.selectedPlaceIDs.contains(place.id)
            ))
        }
    }
    ...
    private func didTapItemCard(at index: Int) {
        guard index < viewModel.places.count else { return }
        viewModel.togglePlaceSelection(viewModel.places[index])
    }

    @objc private func didTapSeeAll() {
        guard let city = viewModel.selectedCity?.name, !viewModel.places.isEmpty else { return }
        guard let presenter = ParentViewController else { return }

        let allPlacesViewController = AllPlacesViewController(city: city, viewModel: viewModel)

        if let navigationController = presenter.navigationController {
            navigationController.pushViewController(allPlacesViewController, animated: true)
        } else {
            presenter.present(UINavigationController(rootViewController: allPlacesViewController), animated: true)
        }
    }
}
